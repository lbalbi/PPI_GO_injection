...
def main():
    ...
    if args.gen:
        ...
        if args.col8: # With only original PPI mean confidence scores from STRING
        ...
        if args.col9: # With PPI mean confidence scores and mean IC SECO score
            x1 = scatter(data_edge1, edge_ind1[0], dim=0,
                       dim_size=data_orignodes, reduce='mean').to('cpu')
            x2 = scatter(data_edge2, edge_ind2[0], dim=0,
                       dim_size=num_node_gen, reduce='mean').to('cpu')
    ...
    if args.fullgo:
        if args.bp: # If per domain testing:
            num_node_go = 176256
        if args.mf:
            num_node_go = 176605
        if args.col8: # With only original PPI mean confidence scores from STRING
    ...
    if args.use_node_embedding: # combine node features with Node2vec embedding
        embedding = torch.load('embedding.pt', map_location='cpu')
        x = torch.cat([x, embedding], dim=-1)
    
    if args.save_embedding: # save embedding for use with GNN
    ...
    model = MLP(x.size(-1), args.hidden_channels, labels, args.num_layers,
